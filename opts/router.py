...
async def send_otp(request: schemas.CreateOTP, db: Session = Depends(get_db)):
    ...
    # # Send OTP to phone
    detail = send_otp_twilio(str(request.phone_number), otp_code)
    return {
        "code": 200,
        "detail": "We've sent an OTP to reset your password.",
        "data": {
...
async def verify_otp(request: schemas.VerifyOTP, db: Session = Depends(get_db)):
    ...
    return {"code": 200, "detail": "OTP verified successfully"}
